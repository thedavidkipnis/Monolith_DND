...
inline std::vector<std::pair<int, int>> findPathForPlayer(Room* room, int startX, int startY, int targetX, int targetY, int maxMovement) {
    const auto& tiles = room->getTiles();
    const int width = room->getWidth();
    const int height = room->getHeight();
    ...
    while (!openList.empty()) {
        ...
        if (current.x == targetX && current.y == targetY) {
            if (current.gCost <= maxMovement) {
                ...
                break; // Can't reach target within movement range
            }
        }
        ...
        for (const auto& [dx, dy] : directions) {
            ...
            // If this tile would put us over our movement speed, skip it
    ...
    return {}; // No path found or not enough movement
}